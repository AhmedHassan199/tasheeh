...
import { X, Award, BookMarked, GraduationCap, Sparkles } from 'lucide-react';
    ...
    <AnimatePresence>
      {teacher && (
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`تفاصيل ${teacher.name}`}
        >
          ...
          >
            <div className="grid gap-0 md:grid-cols-12">
              ...
              <div className="relative md:col-span-7 p-7 sm:p-10">
                <button
                  ...
                  onClick={onClose}
                  aria-label="إغلاق"
                  className="absolute left-4 top-4 grid h-10 w-10 place-items-center rounded-full border border-ink-900/10 dark:border-ink-100/15 text-ink-700 dark:text-ink-200 hover:bg-flame-500 hover:text-white hover:border-transparent transition-colors"
                >
                  <X size={18} />
                </button>

                <span className="hidden md:inline-flex eyebrow">
                ...
                <p className="mt-6 leading-relaxed text-ink-700 dark:text-ink-200">
                  {teacher.bio}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-8 px-7 sm:px-10 pb-2 md:grid-cols-2">
              <DetailBlock icon={GraduationCap} title="تتلمذ على يد" items={teacher.studentOf} />
              <DetailBlock icon={Award} title="الجوائز والإجازات" items={teacher.awards} />
            </div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {teacher.gallery.map((src, i) => (
                  ...
                  >
                    <img
                      src={src}
                      alt={`عمل من أعمال ${teacher.name}`}
                      className="aspect-square w-full object-cover transition-transform duration-700 group-hover:scale-110"
                    />
                    <span className="absolute inset-0 bg-gradient-to-t from-ink-900/60 to-transparent opacity-0 transition-opacity group-hover:opacity-100" />
                  </motion.div>
                ))}
              </div>
            </div>
            ...
            <div className="sticky bottom-0 mt-10 border-t border-ink-900/10 dark:border-ink-100/10 bg-paper/95 dark:bg-[#120A05]/95 backdrop-blur-md px-7 sm:px-10 py-5 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <p className="font-bold text-ink-700 dark:text-ink-200">
                  جاهز لبدء رحلتك مع {teacher.name}؟
                </p>
              </div>
              <button
                type="button"
